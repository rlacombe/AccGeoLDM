...
def train_epoch(args, loader, epoch, teacher, model, model_dp, model_ema, ema, device, dtype, property_norms, optim,
    ...
    for i, data in enumerate(loader):
        ...
        # Sample time steps
        t, u, v, alpha_t, sigma_t, alpha_u, sigma_u, alpha_v, sigma_v = sample_time_steps(teacher, model.timesteps, z_x)
...
def sample_time_steps(model, N, x):
        ...
        gamma_u = model.inflate_batch_array(model.gamma(t), x)
        gamma_v = model.inflate_batch_array(model.gamma(t), x)
...
def encode_to_latent_space(model, x, h, node_mask, edge_mask, context):
        ...
        t_zeros = torch.zeros(size=(x.size(0), 1), device=x.device).to(x.device)

...
def analyze_and_save(args, eval_args, device, generative_model,
    ...
    for i in range(int(n_samples/batch_size)):
        ...
        # TODO maybe turn off this option to measure only generation time
        if save_to_xyz:
    ...
    # TODO need to check generated molecules have the intended number of nodes
    num_nodes_chk = molecules['one_hot'].sum() / len(molecules['one_hot'])
...
def main():
    ...
    # TODO define n_nodes from parsed arguments
    assert eval_args.model_path is not None
    ...
    if prop_dist is not None:
        # NOTE: don't need to update prop_dist if evaluating unconditionally 

        property_norms = compute_mean_mad(dataloaders, args.conditioning, args.dataset)
        prop_dist.set_normalizer(property_norms)
    generative_model.to(device)

    # Modify node distribution to match n_nodes
    # from qm9.models: nodes_dist = Categorical(torch.tensor(prob))
    ...
    # TODO check for intended number of nodes
    #print(f"Number of nodes check: {num_nodes_chk}")
